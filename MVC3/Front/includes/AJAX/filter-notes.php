...
if (isset($_POST['searched_keyword'])) {   
    ...
    if ($_POST['searched_keyword'] != '') {
        ...
        if ($_POST['selected_rating'] != null) {
            ...
            $select_total_notes = "SELECT n.* , AVG(Ratings) AS Ratings FROM seller_notes n JOIN seller_notes_reviews r ON n.NoteID = r.NoteID WHERE n.Status = 9 AND n.IsActive = 1 AND r.IsActive = 1" . $selected_key . " GROUP BY n.NoteID HAVING Ratings " . $ratings;
            $select_notes = "SELECT n.* , AVG(Ratings) AS Ratings FROM seller_notes n JOIN seller_notes_reviews r ON n.NoteID = r.NoteID WHERE n.Status = 9 AND n.IsActive = 1 AND r.IsActive = 1" . $selected_key . " GROUP BY n.NoteID HAVING Ratings " . $ratings . " LIMIT $start_from, $limit";            
        }
    }
