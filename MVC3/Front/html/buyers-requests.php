...
if (isset($_GET['d_id'])) {
    ...
    $NoteID = mysqli_fetch_assoc(mysqli_query($connection, "SELECT NoteID FROM downloads WHERE DownloadID = {$get_download_id} "))['NoteID'];
    ...
    $update_downloads = "UPDATE downloads SET IsSellerHasAllowedDownload = 1, AttachmentPath = '{$all_attachment_path}' WHERE DownloadID = {$get_download_id}";
                            ...
                            <tbody>
                                ...
                                if ($buyer_requests_count != 0) {
                                    while ($row = mysqli_fetch_assoc($buyer_requests_query)) {
                                        ...
                                        $country_code = empty($buyer_details['CountryCode']) ? '-' : "+" . $buyer_details['CountryCode'];
                                        $buyer_email = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM users WHERE UserID = {$downloader_id} "))['EmailID'];
                                        $buyer_phone = empty($buyer_details['PhoneNumber']) ? null : $buyer_details['PhoneNumber'];
                                        ...
                                        $i++;
                                        echo "<td class='purple-td'><a href='note-detail.php?note_id={$note_id}'>$title</a></td>";
                                        echo "<td>$category</td>";
